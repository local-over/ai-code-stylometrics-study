...
PAPER_HTML_PATH = "/home/hassan/Desktop/master_paper_render.html"
PAPER_PDF_PATH = "/home/hassan/Desktop/ai_code_stylometrics_study/paper/research_paper.pdf"

master_markdown = """# Multi-Tier Empirical Analysis of Structural Formatting, Control Complexity, Naming Stylometrics, and Security Vulnerabilities in Human vs. AI Code Synthesis
...
def main():
    ...
    print(f"Master anti-slop research paper written to {PAPER_MD_PATH}")
...
<head>
  <meta charset="UTF-8">
  <title>Master Publication Paper - Hassan Elkady</title>
  <style>
    @page {
      size: A4;
...
<body>
  ...
  <div class="header-container">
    <div class="doc-category">Academic Research Study • 6-Layer Multi-Agent Evaluation</div>
  ...
  <h2>1. Executive Quantitative Findings</h2>
  <p><strong>1. Vertical Whitespace Expansion ("LLM Airiness")</strong>: ChatGPT and DeepSeek-Coder pad every control statement with empty blank lines (16.0% - 20.16% vertical whitespace vs. 0.30% - 3.4% for Humans; U = 3.4 x 10^10, p_adj < 10^-300, r_rb = +0.6817).<br>
  <strong>2. Cyclomatic Complexity Reduction</strong>: Human code exhibits mean Cyclomatic Complexity of 4.11 in Python and 3.84 in Java. LLMs flatten execution into guard-clause paths, reducing CC to 2.12 - 2.66.<br>
  <strong>3. Single-Letter Variable Suppression</strong>: Humans use concise single-letter variables (i, j, k, n) in 28% - 35% of functions. LLMs suppress single-letter variables and enforce strict PEP-8 snake_case (91.05%) or Java camelCase (99.31%) purity.<br>
  <strong>4. Security Flaw Rates</strong>: ChatGPT exhibits a 2.58x higher command injection rate (shell=True, 0.96%) than human developers (0.12%). DeepSeek-Coder commits hardcoded credentials at a 90x higher rate in Java (0.12%).</p>

  <h2>2. 25-Parameter Statistical Benchmark</h2>
  <table>
    <thead>
      <tr>
        <th>Parameter</th>
...
"""
    ...
    print("Converting Master HTML to PDF via Google Chrome...")
    cmd = [
    ...
    if os.path.exists(PAPER_PDF_PATH):
        print(f"Successfully generated Master PDF: {PAPER_PDF_PATH} ({os.path.getsize(PAPER_PDF_PATH):,} bytes)")
