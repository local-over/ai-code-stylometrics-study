...
TYP_PATH = os.path.join(PAPER_DIR, "research_paper.typ")
PDF_PATH = os.path.join(PAPER_DIR, "research_paper.pdf")

typst_markup = """#set page(
  paper: "a4",
...
#align(center)[
  ...
  #v(0.8em)
  #text(10pt, weight: "bold")[Hassan Elkady] \
  #text(9pt, fill: rgb("#334155"))[Computer Engineering Student, Arab Academy for Science, Technology and Maritime Transport (AAST)] \
  #text(8pt, fill: rgb("#64748b"))[August 2026 | Zenodo Multilingual AI Code Dataset: 507,045 Tasks / 2,028,180 Programs Evaluated]
]
...
#rect(
  ...
  #text(9pt, weight: "bold", fill: rgb("#1e293b"))[Abstract] \
  #v(0.4em)
...
def main():
    print("=== Authoring Paper in Typst Scientific Language & Compiling PDF ===")
    with open(TYP_PATH, "w", encoding="utf-8") as f:
        f.write(typst_markup)
    print(f"Authored native Typst document: {TYP_PATH} ({len(typst_markup):,} chars)")

    print("Compiling PDF using Typst 0.15.0 Engine...")
    typst.compile(TYP_PATH, output=PDF_PATH)
    if os.path.exists(PDF_PATH):
        print(f"Successfully generated Typst Vector PDF: {PDF_PATH} ({os.path.getsize(PDF_PATH):,} bytes)")
